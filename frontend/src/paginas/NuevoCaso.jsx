...
export default function NuevoCaso() {
  ...
  const porRol = (rol) => personal.filter(p => p.rol_nombre === rol && p.estado === 'activo')
  ...
  const selectPersonal = (label, key, roles) => {
    const opciones = roles.flatMap(r => porRol(r)).map(p => ({
      value: p.id,
    ...
    <Layout>
      <div style={{ maxWidth: '800px', margin: '0 auto' }}>
        ...
        <div style={{ background: '#fff', borderRadius: '12px', border: '1px solid #e5e7eb', padding: '1.5rem', marginBottom: '1rem' }}>
          <h3 style={{ fontSize: '14px', fontWeight: '700', color: '#0f2d5e', margin: '0 0 1rem', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Datos del caso</h3>
        ...
        <div style={{ display: 'flex', gap: '4px', marginBottom: '1rem' }}>
          {['nna', 'fube'].map(p => (
            <button key={p} onClick={() => p === 'fube' ? handleSiguiente() && setPestana('fube') : setPestana(p)}
              style={{
                padding: '10px 28px', border: 'none', borderRadius: '8px',
                fontSize: '14px', fontWeight: '600', cursor: 'pointer',
                background: pestana === p ? '#0f2d5e' : '#e5e7eb',
                color: pestana === p ? '#fff' : '#6b7280',
                transition: 'all 0.2s',
              }}
        ...
        <div style={{ background: '#fff', borderRadius: '12px', border: '1px solid #e5e7eb', padding: '1.5rem' }}>
          {pestana === 'nna' && (
            <>
              <h3 style={{ fontSize: '14px', fontWeight: '700', color: '#0f2d5e', margin: '0 0 1.25rem', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Datos NNA</h3>
          ...
          {pestana === 'fube' && (
            <>
              <h3 style={{ fontSize: '14px', fontWeight: '700', color: '#0f2d5e', margin: '0 0 1.25rem', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Datos FUBE</h3>
